...
The chat is running on a DigitalOcean droplet at `64.227.181.98`.

| User       | URL                             | Password     |
|------------|---------------------------------|--------------|
| biprarshi  | http://64.227.181.98:5000       | `biprarshi`  |
| protyasha  | http://64.227.181.98:5001       | `protyasha`  |
| shirsa     | http://64.227.181.98:5002       | `shirsa`     |
| subhamoy   | http://64.227.181.98:5003       | `subhamoy`   |
| samrat     | http://64.227.181.98:5004       | `samrat`     |
| padma      | http://64.227.181.98:5005       | `padma`      |
| shreejith  | http://64.227.181.98:5006       | `shreejith`  |
...
### Step 1 — Generate config with `deploy.py`

`deploy.py` takes any number of `NAME:PASSWORD` pairs and generates `docker-compose.prod.yml` and `.env` automatically.

```bash
# Minimum 2 users. Passwords are what users type to log in.
python deploy.py --users alice:secret1 bob:secret2 charlie:secret3

# With a domain (adds Caddy reverse proxy + TLS):
python deploy.py --users alice:secret1 bob:secret2 --domain aqm.example.com

# Custom starting port:
python deploy.py --users alice:secret1 bob:secret2 --base-port 6000

# Generate + immediately start:
python deploy.py --users alice:secret1 bob:secret2 --up
```

This creates:
- **`.env`** — auto-generated DB password, Flask secrets, and your user passwords
- **`docker-compose.prod.yml`** — Redis, PostgreSQL, coin-server, and one container per user
- **`Caddyfile`** (only with `--domain`) — per-user subdomain reverse proxy

Each user gets a sequential port starting from `--base-port` (default 5000). Every user is automatically configured as a contact of every other user.

### Step 2 — Copy files to server and start

```bash
# Copy generated files to your server
scp .env docker-compose.prod.yml root@YOUR_SERVER:/path/to/repo/
...
Or run `deploy.py` directly on the server with `--up`:

```bash
ssh root@YOUR_SERVER "cd /path/to/repo && python deploy.py --users alice:pass bob:pass --up"
```

### Step 3 — Verify

```bash
# Check all containers are running (no "Restarting" status)
docker ps --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}'
```

Each user can then open `http://SERVER_IP:PORT` in a browser and log in with their password.

### Redeploying with different users

Running `deploy.py` again overwrites the previous config. If you change the number of users or passwords:

```bash
# On the server:
docker compose -f docker-compose.prod.yml down
docker volume rm aqm_databse_pg_data    # reset DB if password changed
python deploy.py --users ... --up
```
